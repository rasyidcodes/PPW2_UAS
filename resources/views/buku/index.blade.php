            ...
            <div>
                @if (Auth::check())
                    <a href="{{ route('buku.create') }}"
                        class="inline-block px-4 py-2 border border-blue-500 text-blue-500 bg-blue-100 rounded">
                        {{ __('Tambah Buku Baru') }}
                    </a>
                @endif

                @auth
                    <a href="{{ route('buku.myfavorites') }}"
                        class="inline-block px-4 py-2 border border-green-500 text-green-500 bg-green-100 rounded">
                        {{ __('Buku Favoritku') }}
                    </a>
                @endauth
                ...
                @auth
                    <a href="{{ route('buku.populer') }}"
                        class="inline-block px-4 py-2 border border-orange-500 text-orange-500 bg-orange-100 rounded">
                        {{ __('Buku Populer') }}
                    </a>
                @endauth
            </div>
                            ...
                            @foreach ($data_buku as $buku)
                                <tr class="hover:bg-gray-50">
                                    <th class="flex gap-3 px-6 py-4 font-normal text-gray-900">
                                        ...
                                        @endif
                                        <div class="text-sm">
                                            <a href="{{ $buku->buku_seo ? route('galeri.buku', $buku->buku_seo) : '#' }}"
                                                class="font-medium text-gray-700">{{ $buku->judul }}</a>
                                    ...
                                    <td class="px-6 py-4">
                                        {{ \Carbon\Carbon::parse($buku->tgl_terbit)->format('j F Y') }}</td>
